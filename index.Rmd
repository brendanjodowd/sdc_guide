...
The map above is nice, but we would probably prefer if the legend title said "Population" instead of "Pop2016". Labels for any dimension are easily specified using the ```labs``` function. 

I also want to adjust the appearance of the numbers in the legend so that they have a thousand comma separator. In other ciurcumstances you might want to format the variable with a percent symbol. This is best handled using the [scales](https://scales.r-lib.org/) package, which has some very useful functions for formatting numbers generally. We can add ```label = comma``` to whatever ```scale_fill_...``` function is being used (use ```scale_fill_continuous()``` if no other ```scale_fill_...``` function is used).
...
ggplot(lea_166) + 
  geom_sf(aes(fill=Pop2016)) +
  scale_fill_distiller(palette = "YlOrRd" , label = comma) +
...
Now let's make the inset map. It is ```main_map``` plus ```coord_sf``` to crop to the same coordinates as the black rectangle. Note the use of ```expand = FALSE``` to remove the padding; otherwise the black rectangle would sit a little bit inside the inset plot. I've also added an extra ```theme``` function to get rid of the legend sinmce we don't need an extra legend in the inset. 
